...
1. **네 파트 + 관리 하나**
   ...
   - `3. Paper review/`: 읽은 논문 정리 (세미나 리뷰 + 프로젝트 연구용 레퍼런스)
   - `4. 일상/`: 일상 기록
   - `0. 관리/`: 위 네 파트를 가로지르는 todo·현황 대시보드(`00_Dashboard.md`, 집계 전용) + `Todo.md`·1/2/3 인덱스 노트
...
├── 2. 논문·프로젝트/         # 논문/프로젝트 단위 폴더 (각각 [프로젝트명].md, 진행 요약/ 포함)
├── 3. Paper review/        # 읽은 논문 정리 (세미나 리뷰 [번호. 논문명]/ + 프로젝트별 [프로젝트명] refs/)
├── 4. 일상/                # 일상 기록
...
- **0. 관리**: `00_Dashboard.md`가 Dataview로 전체 todo·최근 활동·프로젝트 현황을 자동 수집(집계 전용, 콘텐츠 없음). 그 외에 `Todo.md`(범용 할 일)와, 1/2/3 각 파트를 요약해 보여주는 인덱스 노트(`1. 개인 공부.md`, `2. 논문·프로젝트.md`, `3. Paper review.md`)를 둠. 상세 규칙: [[0. 관리/CLAUDE]]
- **1. 개인 공부**: 프로젝트에 안 묶인 개인 학습. 주제별 하위 폴더는 필요할 때마다 생성. 상세 규칙: [[1. 개인 공부/CLAUDE]]
- **2. 논문·프로젝트**: 논문/프로젝트마다 폴더 하나. 각 폴더는 `[프로젝트명].md`(폴더명과 동일, 목표/상태/todo), `진행 요약/`(다른 디렉터리에서 진행 중인 실제 작업을 요청 시 스냅샷 요약)로 구성. 새 프로젝트를 만들 땐 `3. Paper review/[프로젝트명] refs/`도 함께 생성해 짝을 맞춘다. 상세 규칙: [[2. 논문·프로젝트/CLAUDE]]
- **3. Paper review**: 읽은 논문 정리. (1) 세미나 리뷰 — 논문 1편당 `[번호. 논문명]/` 폴더, 그 안에 폴더와 동일한 이름의 메인 리뷰 노트 + `refs.md` + 부가 노트. (2) 프로젝트 연구용 레퍼런스 — `[프로젝트명] refs/` 폴더, 논문 1편당 노트 1개. 상세 규칙: [[3. Paper review/CLAUDE]]
