...
1. **세 파트 + 관리 하나**
   ...
   - `3. 일상/`: 일상 기록
   - `0. 관리/`: 위 세 파트를 가로지르는 todo·현황 대시보드 (콘텐츠를 직접 담지 않음)
...
- **0. 관리**: `Dashboard.md`가 Dataview로 전체 todo·최근 활동·프로젝트 현황을 자동 수집. 이 폴더 자체에는 실제 공부/프로젝트 콘텐츠를 두지 않음. 상세 규칙: [[0. 관리/CLAUDE]]
...
- **3. 일상**: 일상 기록, 데일리 노트. 상세 규칙: [[3. 일상/CLAUDE]]
